...
serve(async (req) => {
  ...
  try {
    ...
    const {
      building_id, name, type, description, file_size, file_type, file_url,
      asset_id, provider_id, tags, expires_at,
    } = body;
    ...
    // Verificar acceso al edificio (mínimo technician)
    const role = await requireBuildingAccess(client, user.id, building_id);
    if (role === "viewer") {
      return jsonResponse({ error: "Sin permisos para subir documentos" }, 403);
    }
