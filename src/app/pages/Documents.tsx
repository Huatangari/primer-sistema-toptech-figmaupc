...
import { mockDocuments, mockAssets, mockProviders } from "../../lib/mock-data";
...
export function Documents() {
  ...
  const selectedDoc = mockDocuments.find((d) => d.id === selected);

  const typeCounts = DOC_TYPES.slice(1).map((type) => ({
    type,
    count: mockDocuments.filter((d) => d.type === type).length,
  }));
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filtered.map((doc) => {
                      const asset = mockAssets.find((a) => a.id === doc.assetId);
                      const provider = mockProviders.find((p) => p.id === doc.providerId);
                      ...
                      const isExpired = doc.expiresAt && new Date(doc.expiresAt) < new Date("2025-03-10");
